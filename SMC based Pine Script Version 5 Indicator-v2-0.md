...
8. Adjust action row for buy/sell when the freshness of both zones either buy/sell zone is Used, the Buy/Sell action should change to Wait (also add clock icon in front of the Wait label). \[Done]
9. Proceed with the trend filter (EMA, RSI, Stoch) as what we discussed before. \[Done]
...
1. Trend Filter \[Done]
   ...
   1. Exponential Moving Average (EMA). \[Done]
   2. Relative Strength Index (RSI). \[Done]

      1. RSI parameter : Period = 14, oversold = 35, overbought = 65 \[Todo]
      2. Add checkbox to show/hide RSI on Chart \[Todo]
   3. Stochastic Oscillator (Stoch). \[Done]

      1. Stochastic parameter : Stoch\_K = 5, Stoch\_D = 3, Stoch\_slowing = 3, oversold = 20, overbought = 80 \[Todo]
      2. Add checkbox to show/hide Stoch on Chart \[Todo]
2. Zones \[Done]

   1. Add checkbox to show/hide Zone ZN 1. \[Todo]
   2. Add checkbox to show/hide Zone ZN 2. \[Todo]
3. Dashboard \[Done]

   1. Add option to choose dashboard position (Top Left or Top Right) \[Done]
4. Entry Strategy \[Todo]

   1. Add checkbox to show/hide trade label \[Todo]
   2. Add checkbox to Enable/Disable Zone ZN 1 Entry \[Todo]
   3. Add checkbox to Enable/Disable Zone ZN 2 Entry \[Todo]
   4. Add selection list to choose entry trigger (1. Candle touched, 2. Candle Closed). \[Todo]
5. Take Profit - Stoploss - Breakeven \[Todo]
6. Coming soon \[Hold]
7. Hide Swing High/Low sub section (no need for user to tweak it) \[Done]
8. Disable Zones color setting from Zones sub section \[Done]
9. Remove break of structure sub section (no need for user to tweak it) \[Done]
10. Split the checkbox for trend filter. Each trend filter EMA, RSI, and Stoch should have their own checkbox to enable/disable the filter. \[Todo]
