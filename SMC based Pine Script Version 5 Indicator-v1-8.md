...
6. Adjust Waiting Entry for ZN 1 and ZN 2 with rules below :
   ...
   1. If zone freshness status still "Fresh", the waiting entry status should be "OK". \[Todo]
   2. If zone freshness status already "Used", the waiting entry status should be "NO". \[Todo]
   3. If zone freshness status is "Invalid", the waiting entry status should be "Invalid". \[Todo]
7. Add New row above action row containing 2 columns. The left column label is "SESSION", while the right column should contain either : Asia, Europe, New York, Asia -> Europe, Europe -> New York. \[Todo]
